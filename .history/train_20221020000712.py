...
try:
  ...
  root = '/content/SEGY/python_segy/data/test'
except:
  root = '/kaggle/working/SEGY/python_segy/data/test'

train_data = datagenerator(data_dir = root,patch_size = (256,256),stride = (64,64), train_data_num = 10000,download=True,datasets =1,aug_times=0,scales = [1],verbose=False,jump=1,agc=True)
...
DDataset = DataLoaderTrain(train_dataset,50,{'patch_size': Train['TRAIN_PS']})
VDataset = DataLoaderVal(test_dataset,50,{'patch_size': Train['TRAIN_PS']})
...
for epoch in range(start_epoch, OPT['EPOCHS'] + 1):
    ...
    if epoch % Train['VAL_AFTER_EVERY'] == 0:
        ...
        for ii, data_val in enumerate(val_loader, 0):
            target = data_val[0].cuda().float().repeat(1, 3, 1, 1)
            input_ = data_val[1].cuda().float().repeat(1, 3, 1, 1)
            with torch.no_grad():
