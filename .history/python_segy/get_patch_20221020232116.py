    ...
    for i in range(len(file_list)):
        with segyio.open(file_list[i],'r',ignore_geometry=True) as f:
            ...
            for j in range(0,shot_num,jump):
                ...
                print(f'DATA LENGTH : {len(data)}') #(((W - K + 2P)/S) + 1)
                continue
                q += 1
                if agc:
...
if __name__ == '__main__':
    ...
    train_data  = datagenerator(data_dir = root,patch_size = (96,96),stride = (16,16),train_data_num =1000,download=True,datasets =1,aug_times=9,scales = [1,0.9,0.8],verbose=False,jump=1,agc=False)
