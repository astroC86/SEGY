...
def gen_patches(data,patch_size =(64,64),stride = (32,32), 
                file = 1, file_list = 1,
...
def datagenerator(data_dir:str,patch_size = (128,128), stride = (32,32), 
    ...
    for i in range(len(file_list)):
        with segyio.open(file_list[i],'r',ignore_geometry=True) as f:
            ...
            trace_num = len(sourceX)        # Number of traces, The sourceX under the same shot is the same character.
            ...
            for j in range(0,shot_num,jump):
                data = np.asarray([np.copy(x) for x in f.331[j*len_shot:(j+1)*len_shot]]).T
                print(f'DATA LENGTH : {data.shape}') #(((W - K + 2P)/S) + 1)
                continue
                q += 1
                if agc:
...
if __name__ == '__main__':
    ...
    train_data  = datagenerator(data_dir = root,patch_size = (96,96),stride = (16,16),train_data_num =1000,download=True,datasets =1,aug_times=0,scales = [1], verbose=False,jump=1,agc=False)
